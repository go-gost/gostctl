...
type hostMapperPage struct {
	...
	mappingSelector ui_widget.Selector
...
func NewPage(r *page.Router) page.Page {
	p := &hostMapperPage{
		...
		mappingSelector: ui_widget.Selector{Title: i18n.Hosts},
		...
		delDialog: ui_widget.Dialog{Title: i18n.DeleteHosts},
	}

	return p
}

func (p *hostMapperPage) Init(opts ...page.PageOption) {
	...
	hostMapper, _ := options.Value.(*api.HostsConfig)
	...
	if hostMapper.File != nil || hostMapper.HTTP != nil || hostMapper.Redis != nil {
		p.mode.Value = string(page.AdvancedMode)
	}

	p.name.SetText(hostMapper.Name)
	p.mappings = hostMapper.Mappings
	p.mappingSelector.Clear()
	p.mappingSelector.Select(ui_widget.SelectorItem{Value: strconv.Itoa(len(p.mappings))})

	{
		p.reload.SetText(strconv.Itoa(int(hostMapper.Reload.Seconds())))

		p.enableFileDataSource.SetValue(false)
		if hostMapper.File != nil {
			p.enableFileDataSource.SetValue(true)
			p.filePath.SetText(hostMapper.File.Path)
		}

		p.enableRedisDataSource.SetValue(false)
		if hostMapper.Redis != nil {
			p.enableRedisDataSource.SetValue(true)
			p.redisAddr.SetText(hostMapper.Redis.Addr)
			p.redisDB.SetText(strconv.Itoa(hostMapper.Redis.DB))
			p.redisPassword.SetText(hostMapper.Redis.Password)
			p.redisKey.SetText(hostMapper.Redis.Key)
		}

		p.enableHTTPDataSource.SetValue(false)
		if hostMapper.HTTP != nil {
			p.enableHTTPDataSource.SetValue(true)
			p.httpURL.SetText(hostMapper.HTTP.URL)
			p.httpTimeout.SetText(strconv.Itoa(int(hostMapper.HTTP.Timeout.Seconds())))
		}
	}

	{
		...
		if hostMapper.Plugin != nil {
			p.mode.Value = string(page.PluginMode)
			for i := range page.PluginTypeOptions {
				if page.PluginTypeOptions[i].Value == hostMapper.Plugin.Type {
					p.pluginType.Select(ui_widget.SelectorItem{Key: page.PluginTypeOptions[i].Key, Value: page.PluginTypeOptions[i].Value})
					break
				}
			}
			p.pluginAddr.SetText(hostMapper.Plugin.Addr)

			if hostMapper.Plugin.TLS != nil {
				p.pluginEnableTLS.SetValue(true)
				p.pluginTLSSecure.SetValue(hostMapper.Plugin.TLS.Secure)
				p.pluginTLSServerName.SetText(hostMapper.Plugin.TLS.ServerName)
			}
		}
	}
}

func (p *hostMapperPage) Layout(gtx page.C) page.D {
			...
			return layout.UniformInset(8).Layout(gtx, func(gtx page.C) page.D {
				return layout.Flex{
					...
					layout.Flexed(1, func(gtx page.C) page.D {
						title := material.H6(th, i18n.Hosts.Value())
		...
		return layout.UniformInset(16).Layout(gtx, func(gtx page.C) page.D {
			return layout.Flex{
				...
				layout.Rigid(func(gtx page.C) page.D {
					...
					if p.mappingSelector.Clicked(gtx) {
						perm := page.PermRead
						if p.edit {
							perm = page.PermWrite | page.PermDelete
						}
						p.router.Goto(page.Route{
							Path:     page.PageHostMapping,
							ID:       uuid.New().String(),
							Value:    p.mappings,
							Callback: p.callback,
							Perm:     perm,
					...
					return p.mappingSelector.Layout(gtx, th)
				}),
				layout.Rigid(func(gtx page.C) page.D {
...
func (p *hostMapperPage) layoutDataSource(gtx page.C, th *page.T) page.D {
	if p.mode.Value != string(page.AdvancedMode) {
		return page.D{}
	}
...
func (p *hostMapperPage) showPluginTypeMenu(gtx page.C) {
	for i := range page.PluginTypeOptions {
...
func (p *hostMapperPage) callback(action page.Action, id string, value any) {
	if id == "" {
		return
	}
	...
	case page.ActionUpdate:
		p.mappings, _ = value.([]*api.HostMappingConfig)
...
func (p *hostMapperPage) save() bool {
	cfg := p.generateConfig()

	var err error
	if p.id == "" {
		err = runner.Exec(context.Background(),
			task.CreateHostMapper(cfg),
			runner.WithCancel(true),
		)
	} else {
		err = runner.Exec(context.Background(),
			task.UpdateHostMapper(cfg),
			runner.WithCancel(true),
		)
	}
...
func (p *hostMapperPage) generateConfig() *api.HostsConfig {
	cfg := &api.HostsConfig{
...
func (p *hostMapperPage) delete() {
	runner.Exec(context.Background(),
		task.DeleteHostMapper(p.id),
		runner.WithCancel(true),
	)
	util.RestartGetConfigTask()
}
