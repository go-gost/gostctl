...
type hostMapperList struct {
	router *page.Router
	list   layout.List
	states []state
}

func HostMapper(r *page.Router) List {
	return &hostMapperList{
		router: r,
		list: layout.List{
...
func (p *hostMapperList) Layout(gtx page.C, th *page.T) page.D {
	cfg := api.GetConfig()
	mappers := cfg.Hosts

	if len(mappers) > len(p.states) {
		states := p.states
		p.states = make([]state, len(mappers))
		copy(p.states, states)
	}

	return p.list.Layout(gtx, len(mappers), func(gtx page.C, index int) page.D {
		if p.states[index].clk.Clicked(gtx) {
			p.router.Goto(page.Route{
				Path: page.PageHosts,
				ID:   mappers[index].Name,
						...
						layout.Rigid(func(gtx page.C) page.D {
							return layout.Flex{
								...
								layout.Flexed(1, func(gtx page.C) page.D {
									label := material.Body1(th, mapper.Name)
