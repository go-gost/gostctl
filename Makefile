...
android:
	gogio -x -work -target android -minsdk 22 -targetsdk 33 -version $(VERSION).1 -name GOST -signkey build/sign.keystore -signpass android -appid gost.run -o $(BINDIR)/$(NAME)-$(VERSION).aab .
	gogio -x -work -target android -minsdk 22 -targetsdk 33 -version $(VERSION).1 -name GOST -signkey build/sign.keystore -signpass android -appid gost.run -o $(BINDIR)/$(NAME)-$(VERSION).apk .
